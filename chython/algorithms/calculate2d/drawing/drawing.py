...
class Drawer:
    ...
    #             if not neighbour._adjacent_to_stereobond or len(neighbour.draw.rings) < 2 and neighbour.draw.bridged_ring is None or \
    #                     (neighbour.draw.bridged_ring is not None and len(neighbour.draw.original_rings) < 2):
    #                 atom.draw.is_drawn = False
    #             else:
        ...
        if not self.options.draw_hydrogens:
            ...
            for atom in self.structure.graph:
                ...
                if atom.type!= 'H':
                    ...
                    continue
                neighbour = atom.neighbours[0]  # Assuming the first neighbour is the one the hydrogen is attached to
                neighbour.draw.has_hydrogen = True
                if neighbour._adjacent_to_stereobond:
                    exposed.append(atom) # подвергнутый атом??
                else:
                    atom.draw.is_drawn = False
                    hidden.append(atom) # скрытые атомы
            for atom in self.structure.graph:
                atom.set_drawn_neighbours()
                if atom.type == 'O':
                    pass
